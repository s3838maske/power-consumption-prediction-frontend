import { Zap, TrendingUp, Bell, Cpu } from "lucide-react";
...
const COLORS = [
  "hsl(199, 89%, 48%)", "hsl(152, 69%, 41%)", "hsl(37, 95%, 55%)",
...
const UserDashboard = () => {
  ...
  if (statsLoading || peakLoading || comparisonLoading) {
    return <div className="flex h-96 items-center justify-center">Loading dashboard data...</div>;
  }

  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader title="Dashboard" description="Overview of your energy consumption and predictions" />
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-lg border border-border bg-card p-5 shadow-card">
          <h3 className="mb-4 text-sm font-semibold text-card-foreground">Monthly Consumption Comparison</h3>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={comparison}>
              ...
              <Bar dataKey="previous" fill="hsl(220, 15%, 80%)" radius={[4, 4, 0, 0]} name="Previous Period" />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        {/* Peak Hours */}
        <div className="rounded-lg border border-border bg-card p-5 shadow-card">
          <h3 className="mb-4 text-sm font-semibold text-card-foreground">Peak Hours Analysis</h3>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={peakHours}>
              ...
              />
              <Bar dataKey="value" fill="hsl(199, 89%, 48%)" radius={[4, 4, 0, 0]} name="kWh" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        {/* Device Usage Pie */}
        <div className="rounded-lg border border-border bg-card p-5 shadow-card">
          <h3 className="mb-4 text-sm font-semibold text-card-foreground">Device Usage Distribution</h3>
        ...
        <div className="rounded-lg border border-border bg-card p-5 shadow-card lg:col-span-2">
          <h3 className="mb-4 text-sm font-semibold text-card-foreground">Daily Consumption Trend (Last 30 Days)</h3>
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={stats?.dailyData || []}>
              ...
              />
              <Area type="monotone" dataKey="consumption" stroke="hsl(199, 89%, 48%)" strokeWidth={2.5} fill="url(#colorConsumption)" name="kWh" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
