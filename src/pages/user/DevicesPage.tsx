...
const DevicesPage = () => {
    ...
    onError: (err: any) => {
      ...
      if (typeof errorData === "object") {
        Object.keys(errorData).forEach((key) => {
          toast.error(`${key}: ${errorData[key].join(", ")}`);
        });
      } else {
  ...
  const resetForm = () => {
    setForm({ name: "", device_type: "", location: "", power_rating: "" });
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload = {
      ...form,
      power_rating: parseFloat(form.power_rating) || 0,
    };
    if (editingId) {
  ...
  const filtered =
    (Array.isArray(devices) ? devices : devices?.results)?.filter(
      (d: any) =>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, power_rating: e.target.value })
                }
                  ...
                  filtered.map((device: any) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-sm font-medium text-foreground">
                        {device.power_rating?.toLocaleString()}
                      </td>
                      <td className="px-4 py-3">
                        <span
                        ...
                        >
                          <span
                            className={`h-1.5 w-1.5 rounded-full ${device.status === "active" ? "bg-success" : "bg-muted-foreground"}`}
                          />
                          {device.status}
                        </span>
                      </td>
