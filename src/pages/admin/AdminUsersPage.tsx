...
const AdminUsersPage = () => {
  ...
  const activateMutation = useMutation({
    mutationFn: async ({ id, activate }: { id: string, activate: boolean }) => {
      // In a real app we might have a deactivate endpoint, here we just use the activate one
  ...
  const filtered = users?.results?.filter((u: any) => 
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.map((user: any) => (
                    <tr key={user.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-sm text-muted-foreground">{new Date(user.date_joined || user.created_at).toLocaleDateString()}</td>
                      <td className="px-4 py-3">
                        <button 
                          onClick={() => activateMutation.mutate({ id: user.id, activate: user.status !== 'active' })}
