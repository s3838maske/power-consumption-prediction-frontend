  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader title="Categories" description="Manage device categories">
        ...
        >
          {showForm ? "Close Panel" : <><Plus className="h-4 w-4" /> Add Category</>}
        </button>
      </PageHeader>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {categories?.length === 0 ? (
            ...
            categories?.map((cat: any) => (
              <div key={cat.id} className="rounded-lg border border-border bg-card p-5 shadow-card hover:shadow-elevated transition-shadow">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-semibold text-card-foreground">{cat.name}</h4>
                      <p className="text-xs text-muted-foreground truncate max-w-[150px]">{cat.description || 'No description'}</p>
                    </div>
                  </div>
                ...
                <div className="mt-3 pt-3 border-t border-border flex justify-between items-center">
                  <p className="text-xs text-muted-foreground">{cat.device_count || 0} devices</p>
                  <span className="text-[10px] text-muted-foreground">ID: {cat.id}</span>
                </div>
              </div>
            ))
          )}
